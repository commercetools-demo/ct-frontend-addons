import { ActionHandler, DataSourceConfiguration, DataSourceContext, DataSourceResult, DynamicPageContext, DynamicPageRedirectResult, DynamicPageSuccessResult, Request } from '@frontastic/extension-types';
...
export type UnionDependencies = MinimumQuantityDependencies | SuperuserDependencies | SuperuserB2BDependencies | ConfigurableProductsDependencies | ApprovalWorkflowsDependencies;
...
export type MergableDynamicHandlers<T> = (request: Request, context: DynamicPageContext, originalResult: DynamicPageSuccessResult, config: T) => Promise<DynamicPageSuccessResult | DynamicPageRedirectResult | null>;
export type NewDynamicHandlers<T> = (request: Request, context: DynamicPageContext, config: T) => Promise<DynamicPageSuccessResult | DynamicPageRedirectResult | null>;
...
export type DatasourceSig = (config: DataSourceConfiguration, context: DataSourceContext) => Promise<DataSourceResult> | DataSourceResult;
...
export type DynamicPagehandler = (request: Request, context: DynamicPageContext) => Promise<DynamicPageSuccessResult | DynamicPageRedirectResult | null>;
