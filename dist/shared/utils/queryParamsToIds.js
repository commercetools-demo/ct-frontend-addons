...
function queryParamsToIds(param, queryParams) {
    var paramIds = [];
    var requestParamIds = queryParams === null || queryParams === void 0 ? void 0 : queryParams[param];
    if (requestParamIds) {
        if (Array.isArray(requestParamIds)) {
            paramIds.push.apply(paramIds, requestParamIds);
        }
        else {
            var params = requestParamIds.split(',');
            if (params.length > 1) {
                paramIds.push.apply(paramIds, params);
            }
            else {
