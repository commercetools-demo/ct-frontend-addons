 ...
 * queryParamsToStates('status', queryParams); // returns ["active", "inactive"]
 */
var queryParamsToStates = function (param, queryParams) {
    var states = [];
    var requestParamStates = queryParams === null || queryParams === void 0 ? void 0 : queryParams[param];
    if (requestParamStates) {
        if (Array.isArray(requestParamStates)) {
            states.push.apply(states, requestParamStates);
        }
        else {
            var params = requestParamStates.split(',');
            if (params.length > 1) {
                states.push.apply(states, params);
            }
            else {
