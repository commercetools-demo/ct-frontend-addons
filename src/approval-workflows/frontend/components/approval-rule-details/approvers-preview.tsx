...
const ApproversPreview: React.FC<Props> = ({
  ...
  return (
    <div className="border border-primary-300 p-2">
      <span>{translate('dashboard.rule.details.approvers')}</span>
                  ...
                  {and.or?.map((or, index) => (
                    <TreeNode key={index} label="OR">
                      <div className="mx-auto mt-1 flex items-center border border-accent-300 p-1">
                        <span>{or.associateRole.key}</span>
      ...
      {!isPreview && (
        <div className="mt-2 w-full border-t">
          <components.Button variant="primary" size="s" type="button" onClick={onEdit}>
            {translate('dashboard.rule.details.edit')}
          </components.Button>
        </div>
      )}
    </div>
